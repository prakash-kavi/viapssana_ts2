"""Generate 2D and 3D attractor visualisations from the tail of the training rollouts."""
...
def plot_attractor_2d(
    ...
    for ax, cohort_data, title in zip(axes, (novice, expert), ("Novice", "Expert")):
        ...
        if fe.size:
            fe_norm = (fe - fe.min()) / (fe.ptp() + 1e-10)
        else:
    ...
    cbar.set_label("Free energy (relative scale)", fontweight="bold")
...
def plot_attractor_landscape_3d(
    ...
    nov_surface = _kernel_surface(nov_x, nov_y, nov_fe, grid_x, grid_y, bandwidth)
    exp_surface = _kernel_surface(exp_x, exp_y, exp_fe, grid_x, grid_y, bandwidth)

    combined = np.array([val for val in [nov_surface, exp_surface] if np.isfinite(val).any()])
    if combined.size:
        z_min = float(np.nanmin(combined))
        z_max = float(np.nanmax(combined))
    else:
        z_min, z_max = 0.0, 1.0
    ...
    for ax in (ax_left, ax_right):
        ...
        ax.set_zlabel("Free energy", labelpad=8, fontweight="bold")
        ax.grid(False)
        ax.set_zticks([])
